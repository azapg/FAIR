...
class TestModelRelationships:
    ...
    def test_user_course_assignment_relationships(self, test_db):
        ...
        with test_db() as session:
            ...
            assert course.instructor is not None, (
                "Course should have instructor relationship"
            )
            assert course.instructor.id == professor_id, (
                "Course instructor should be the professor we created"
            )
            assert course.instructor.name == "Prof. Jane Smith", (
                "Instructor name should match"
            )

            assert assignment.course is not None, (
                "Assignment should have course relationship"
            )
            assert assignment.course.id == course_id, (
                "Assignment course should be the course we created"
            )
            assert assignment.course.name == "Advanced Python Programming", (
                "Course name should match"
            )

            assert len(professor.courses) > 0, (
                "Professor should have courses relationship"
            )
            assert professor.courses[0].id == course_id, (
                "Professor's course should be the one we created"
            )

            assert len(course.assignments) > 0, (
                "Course should have assignments relationship"
            )
            assert course.assignments[0].id == assignment_id, (
                "Course's assignment should be the one we created"
            )
            assert course.assignments[0].title == "Implement a Web API", (
                "Assignment title should match"
            )
    ...
    def test_foreign_key_constraints(self, test_db):
        ...
        with test_db() as session:
            ...
            try:
                session.commit()
                assert False, (
                    "Expected IntegrityError when creating course with invalid instructor_id"
                )
            ...
            try:
                session.commit()
                assert False, (
                    "Expected IntegrityError when creating assignment with invalid course_id"
                )
            except IntegrityError:
                session.rollback()
                pass
    ...
    def test_user_role_validation(self, test_db):
        ...
        with test_db() as session:
            ...
            assert roles_found == expected_roles, (
                f"Expected roles {expected_roles}, found {roles_found}"
            )
    ...
    def test_assignment_deadline_validation(self, test_db):
        ...
        with test_db() as session:
            ...
            course = Course(id=uuid4(), name="Test Course", instructor_id=professor.id)
            session.add(course)
            session.commit()
            ...
            assert isinstance(assignment.deadline, datetime), (
                "Deadline should be datetime object"
            )

            future_assignments = (
                session.query(Assignment)
                .filter(Assignment.deadline > datetime.now())
                .all()
            )
            assert len(future_assignments) > 0, (
                "Should find assignments with future deadlines"
            )
            assert assignment.id in [a.id for a in future_assignments], (
                "Our assignment should be in future assignments"
            )
