...
@FairPlugin(
    id="up.ac.pa.allan.zapata.simple_transcriber",
    name="SimpleTranscriber",
    version="1.0.0",
    author="Test Author",
    description="A simple transcriber plugin.",
)
class SimpleTranscriber(TranscriptionPlugin, ABC):
    instructions = TextField(
        label="Instructions",
        required=True,
        default="Transcribe the following math problem.",
    )
...
@FairPlugin(
    id="up.ac.pa.allan.zapata.simple_grader",
    name="SimpleGrader",
    version="1.0.0",
    author="Test Author",
)
class SimpleGrader(GradePlugin, ABC):
    ...
    def grade(self, transcription: TranscribedSubmission) -> GradeResult:
        ...
        return GradeResult(score=0.5, feedback="Lenient grading applied.", meta={})


# TODO: What happens if a plugin implements multiple plugin types?
@FairPlugin(
    id="up.ac.pa.allan.zapata.complex_plugin",
    name="ComplexPlugin",
    version="1.0.0",
    author="Test Author",
)
class ComplexPlugin(TranscriptionPlugin, ABC):
    instructions = TextField(
        label="Instructions",
        required=True,
        default="Transcribe and grade the following content.",
    )
    strict = SwitchField(label="Strict Grading", required=False, default=False)
    max_tokens = NumberField(
        label="Max Tokens", required=False, default=1000, ge=100, le=2000
    )
    temperature = NumberField(
        label="Temperature", required=False, default=0.7, ge=0.0, le=1.0
    )
