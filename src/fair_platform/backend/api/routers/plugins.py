...
def grade(
    plugin_id: str, grade_request: GradeRequest, user: User = Depends(get_current_user)
):
    if user.role != UserRole.admin and user.role != UserRole.professor:
        raise HTTPException(
            status_code=403, detail="Not authorized to grade using plugins"
        )
    ...
    original_submission = Submission(
        id=grade_request.submission_id,
        submitter=submitter,
        submitted_at="",
        assignment=assignment,
        artifacts=[],
    )
    transcribed_submission = TranscribedSubmission(
